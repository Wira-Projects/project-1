...
// Tipe DebugInfo & ApiErrorResponse bisa disalin/dibuat ulang
interface DebugInfo {
    invoked: boolean;
...
export default defineEventHandler(async (event: H3Event): Promise<ApiSuccessResponse | ApiErrorResponse> => {
    ...
    const debugInfo: DebugInfo = {
        ...
        step: 'Initializing Marketplace PATCH',
        modelId: modelId,
    ...
    if (!modelId) {
        ...
        return {
            error: createError({ statusCode: 400, statusMessage: debugInfo.errorMessage }).toJSON(),
            debug: debugInfo
        };
    }
    ...
    try {
        ...
        if (!debugInfo.expectedAdminEmail) {
           // ... (Error handling sama seperti GET)
        ...
        return {
            success: true,
            message: `Model ${modelId} availability updated successfully.`,
            debug: debugInfo
        };
