             ...
             <table class="w-full text-left text-sm whitespace-nowrap">
               ...
               <tbody class="divide-y divide-slate-700">
                     ...
                     <tr v-else-if="!marketplaceModels || marketplaceModels.length === 0">
                       <td colspan="6" class="p-4 text-center text-slate-500">
                         Tidak ada data model AI di database.
                       </td>
                     </tr>
       ...
       <div v-if="activeTab === 'providers'">
         <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div class="card p-6 rounded-lg">
             <h2 class="text-xl font-bold mb-4 text-amber-400 flex items-center">
                <i class="fas fa-key mr-3"></i> OpenRouter API Keys
             </h2>
             <p class="text-sm text-slate-400 mb-4">Buat API Key baru di OpenRouter melalui API Provisioning.</p>
             ...
             <form @submit.prevent="submitNewOpenRouterKey">
               ...
               <div class="mb-4">
                 ...
                 <p class="text-xs text-slate-500 mt-1">Isi angka dalam cents. Misal: 1000 untuk $10.00</p>
               </div>

               <p v-if="addOpenRouterKeyError" class="error-banner text-sm mb-4">{{ addOpenRouterKeyError }}</p>
                <p v-if="addOpenRouterKeySuccess" class="success-banner text-sm mb-4">
                    {{ addOpenRouterKeySuccess }}
                    <span class="block mt-1 font-mono text-xs bg-slate-600 px-2 py-1 rounded break-all">{{ lastCreatedKeyLabel }}</span>
                </p>
           ...
           <div class="card p-6 rounded-lg">
              <h2 class="text-xl font-bold mb-4 flex items-center">
                 <i class="fas fa-server mr-3"></i> Manajemen Penyedia Lain
              </h2>
              <p class="text-sm text-slate-400 mb-4">Tambahkan atau edit detail penyedia API (Groq, Together AI, dll) di database.</p>
              <button class="w-full bg-slate-600 hover:bg-slate-500 text-slate-200 font-semibold py-2 px-4 rounded-lg text-sm transition disabled:opacity-50" disabled>
                  Tambah Penyedia Baru (TBD)
              </button>
           </div>
         </div>
       </div>
                 ...
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Nama persis (mis: OpenRouter)"
                            required
...
// Interface untuk response sukses dari API OpenRouter Key POST
interface OpenRouterKeySuccessResponse { success: true; message: string; newKeyData: OpenRouterKey; debug: any;}
...
const newModelData = reactive({
    provider_name: '', // Menggunakan nama provider
    provider_model_id: '',
    display_name: '',
    model_type: '',
...
const newOpenRouterKeyData = reactive({
    name: '',
...
const errorResponse = computed(() => {
    if (fetchError.value) {
         ...
         // Buat objek debug fallback jika tidak ada di error
...
const formatCurrency = (amount: number | null | undefined, currency = 'USD', maximumFractionDigits = 2): string => {
  if (amount === null || amount === undefined || isNaN(amount)) return `${currency} 0.00`; // Default ke 0.00
...
const submitNewModel = async () => {
    ...
    addModelError.value = null;
    const payload = {
        provider_name: newModelData.provider_name,
    ...
    try {
        const result = await $fetch('/api/admin/marketplace', { method: 'POST', body: payload });
        // @ts-ignore
    ...
    } finally { isAddingModel.value = false; }
};
...
const toggleAvailability = async (model: MarketplaceModel, event: Event) => {
    ...
    try {
        ...
        // Optimistic update pada data lokal sebelum refresh (opsional)
        const localModel = marketplaceModels.value.find(m => m.id === modelId);
        if (localModel) localModel.is_available = newAvailability;
        await refresh(); // Refresh untuk memastikan konsistensi
        await new Promise(resolve => setTimeout(resolve, 50)); // Jeda singkat
    } catch (err: any) {
        console.error('Failed update:', err); alert(`Gagal: ${err.data?.message || err.message}`); target.checked = !newAvailability;
        // Rollback data lokal jika update gagal
        const localModel = marketplaceModels.value.find(m => m.id === modelId);
        if (localModel) localModel.is_available = !newAvailability;
        // Tidak perlu refresh lagi di sini karena sudah rollback
    } finally {
        // Hapus status loading setelah sedikit jeda agar UI sempat update
        setTimeout(() => { delete togglingStatus[modelId]; }, 150);
    }
};

const submitNewOpenRouterKey = async () => {
    isAddingOpenRouterKey.value = true;
    addOpenRouterKeyError.value = null;
    addOpenRouterKeySuccess.value = null;
    lastCreatedKeyLabel.value = null;
    const payload = {
        name: newOpenRouterKeyData.name,
        limit: newOpenRouterKeyData.limit && newOpenRouterKeyData.limit > 0 ? newOpenRouterKeyData.limit : undefined,
    };
    try {
        // Explicitly type the expected success response
        const result = await $fetch<OpenRouterKeySuccessResponse>('/api/admin/openrouter/keys', {
            method: 'POST',
            body: payload,
        });
        // Type guard is less necessary now, but check success property
        if (result && result.success) {
            addOpenRouterKeySuccess.value = result.message;
            lastCreatedKeyLabel.value = result.newKeyData.label;
            newOpenRouterKeyData.name = '';
            newOpenRouterKeyData.limit = null;
        } else {
             // Handle cases where API might return success: false or unexpected structure
             // @ts-ignore
             throw new Error(result?.message || 'Gagal membuat OpenRouter key.');
        }
    } catch (err: any) {
        addOpenRouterKeyError.value = err.data?.message || err.message || 'Terjadi kesalahan saat membuat key.';
        console.error("Error creating OpenRouter key:", err);
    } finally {
        isAddingOpenRouterKey.value = false;
    }
};
...
/* Style untuk key label di success banner */
.success-banner code {
